...
웹사이트 코드는 즉시 브라우저에서 실행 가능해야 하며, 모든 기능이 정상적으로 작동해야 합니다.`
    ...
    artist_ai_create: {
        ...
        user_prompt_template: `다음 설명에 기반한 웹사이트용 이미지를 생성해 주세요:

[{image_description}]

다음 사항을 고려해 최적의 이미지를 생성해 주세요:
1. 위 설명에 정확히 부합하는 고품질 이미지를 생성합니다.
2. 이미지는 웹사이트에 사용될 것이므로 전문적인 느낌과 높은 해상도가 필요합니다.
3. 텍스트는 가급적 포함하지 마세요 (로고나 간단한 레이블이 필요한 경우 제외).
4. 이미지 비율은 웹사이트에 적합한 16:9 또는 4:3 비율이 좋습니다.
5. 저작권 문제가 없어야 합니다.

이미지가 생성되면 다음 정보를 JSON 형식으로 응답해 주세요:
1. url: 생성된 이미지의 URL
2. alt_text: 이미지에 대한 대체 텍스트 (접근성을 위함)
3. desc: 생성된 이미지에 대한 자세한 설명 (최대 300자)

이미지 생성에 사용된 모델과 설정도 같이 알려주세요.`
    },
    artist_ai_search: {
...
5. 가능하면 다양한 옵션(3-5개)을 제공해 주세요.

각 이미지에 대해 다음 정보를 JSON 형식으로 제공해 주세요:
1. url: 이미지 직접 링크
